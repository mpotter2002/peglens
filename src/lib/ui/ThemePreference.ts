...
export class ThemePreference {
  ...
  static blockingScript(): string {
    ...
    const fallback = JSON.stringify(ThemePreference.DEFAULT);
    return `(function(){try{var k=${key};var v=localStorage.getItem(k);var c=v==="light"||v==="dark"||v==="system"?v:${fallback};var d=c==="dark"||(c==="system"&&window.matchMedia("(prefers-color-scheme: dark)").matches);var r=document.documentElement;r.classList.toggle("dark",d);r.style.colorScheme=d?"dark":"light"}catch(e){}})();`;
  }
}
